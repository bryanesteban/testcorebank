...
public class ClienteServiceImp implements ClienteServInterface {
    ...
    public List<ClienteDTO> findAll() {
      ...
      try {
       ...
       return clientes
                .stream()
                .map(u -> ClienteDTO.build(decryptUser(u)))
    ...
    public Optional<ClienteDTO> findByIdClient(String clienteId) {
        ...
        try {
            clienteDTO = clienteRepository
                    ...
                    .map(u -> {
                       return ClienteDTO.build(decryptUser(u));
                    });
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ClienteDTO saveClient(Cliente cliente) {
        try {
            ...
            if(!personaValidation.isPresent()){
                ...
                return ClienteDTO.build(decryptUser(clienteRepository.save(encryptUser(cliente))));
            }
            return null;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Optional<ClienteDTO> update(Cliente cliente, String clienteId) {
        try {
            ...
            if (clienteOpt.isPresent()) {
                Cliente existingCliente = decryptUser(clienteOpt.get());
                ...
                // Guarda los cambios
                clienteRepository.save(encryptUser(existingCliente));
    ...
    //Encripta clientes 
    public Cliente  encryptUser (Cliente cliente){
    ...
    public Cliente decryptUser (Cliente cliente) {
