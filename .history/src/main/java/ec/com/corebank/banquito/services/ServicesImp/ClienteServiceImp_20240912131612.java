...
public class ClienteServiceImp implements ClienteServInterface {
    ...
    public Cliente decryptCliente (Cliente cliente) {
        ...
        try {
            ...
            clientedec.setIdentificacion(cliente.getPersona().getIdentificacion());
            clientedec.setEstado(cliente.getEstado());

            clientedec.setIdentificacion(cliente.getPersona().getIdentificacion());
            clientedec.setNombre(encryptServ.decrypt(cliente.getPersona().getNombre()));
            clientedec.setTelefono(encryptServ.decrypt(cliente.getPersona().getTelefono()));
            clientedec.setDireccion(encryptServ.decrypt(cliente.getPersona().getDireccion()));
            clientedec.setGenero(cliente.getPersona().getGenero());
            clientedec.setEdad(cliente.getPersona().getEdad());
